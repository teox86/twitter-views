...
!(function (context, definition) {
  if (typeof define == 'function' && typeof define.amd  == 'object') define(['jquery'], definition);
    ...
    $.fn.counter = function(options) {
        ...
        var tick = function(thisContext) {
            var e = $(thisContext);
            var data = e.data('counter');
            var i = data.parts.length - 1;
            while(i >= 0 ) {
                var part = data.parts[i];
                part.value += data.down ? -1 : 1; //Increase/decrease by 1 the value of the data.parts[i]
        ...
        return this.each(function() {
            ...
            data.interval = parseInt(options.interval || e.attr('data-interval') || '1000', 10); // (10 = explicit decimal)
            //data.down = ( options.direction || e.attr('data-direction') || 'down') == 'down';
            data.down = false;
            data.parts = []; // create data.parts
            ...
            $.each(format, function(index, value) { // each part of "format" number (e.g. "999", ".", "9")
                if (/^\d+$/.test(value)) {	// if it's a number
                    var part = {};
                    part.index = index;
                    ...
                    var epart = $('<span>').addClass('part').addClass('part' + index); // add one digit per <span> tag
                    // Create digits, if needed, with padding
                    var digits = part.value + '';
                    while (digits.length < part.padding) {
                        digits = '0' + digits;
                    }
                	...
                	// Add separator (as appears in data-format)
            ...
            /* IMPORTANT:
	            Every 500 ms get txt file from servlet and compare with the current value.
	            If changed: tick and update local value.
            */
